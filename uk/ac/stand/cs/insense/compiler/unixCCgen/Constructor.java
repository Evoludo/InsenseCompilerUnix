...
public class Constructor extends DeclarationContainer implements IConstructor {
	...
	public Constructor( IProcedureContainer container, FunctionType ft, List<String> parameter_names, IDeclarationContainer parent, int scope_level ) {
		...
		if(ft.getArgs().size() > 0){
			...
			for( ITypeRep param : ft.getArgs() ) {
				...
				if(!param.isPointerType()) { 
					sb.append(TAB + insenseTypeToCTypeName(param) + SPACE + STAR + "_p2" + parameter_names.get(i) + SPACE + EQUALS_ + functionCall("DAL_alloc", functionCall("sizeof", insenseTypeToCTypeName(param)), SPACE + "false") + SEMI + NEWLINE);
			...
			sb.append(TAB + VOID_ + STAR+STAR+"_argv" + SPACE + EQUALS_ + functionCall("DAL_alloc", parameter_names.size() + STAR + functionCall("sizeof", VOIDSTAR_), SPACE + "false") + SEMI + NEWLINE);
	...
	public void complete(){
		...
		Diagnostic.trace(DiagnosticLevel.FINAL, "constructor " + name + ": call_overhead = " + constructor_call_overhead + ", stack_usage = " + get_maximal_stack_usage());
		super.complete();
	}
