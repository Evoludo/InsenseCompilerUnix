...
public class Function extends DeclarationContainer implements IFunction {
	...
	private ArrayList<IDecl> localDeclsInScope;
...
//		}
	...
	public String generateCode(IProcedureContainer container) {
		...
		if(!ft.getResult().equals(VoidType.TYPE)){ // i.e. function returns a value
			sb.append( TAB + insenseTypeToCTypeName(ft.getResult()) + SPACE + C_return_param_name + SPACE + EQUALS_ + ft.getResult().getDefaultCValue() + SEMI + NEWLINE);
		}
		int i = 0;
		for( ITypeRep param : ft.getArgs() ) {
	...
	public void complete(){
		...
		if(!ft.getResult().equals(VoidType.TYPE) && !hasReturn){
			Cgen.get_instance().getCompilerErrors().generalError("Procedure " + this.name + " has return type " + ft.getResult().toHumanReadableString() + " but no return statement");
		}
	}
