...
import uk.ac.stand.cs.insense.compiler.unixCCgen.fileHandling.FileTracker;
import uk.ac.stand.cs.insense.compiler.unixCCgen.fileHandling.OutputFile;
import uk.ac.standrews.cs.nds.util.ErrorHandling;

/**
 * @author al
 * Generates a Makefile suitable for inclusion with the Contiki system
 *
 */
public class MakefileUnix implements IMakefile {

	private static String name = "Makefile";
	private static String CONTIKI = "CONTIKI = $(INSENSE_RUNTIME_UNIX)";
	private static String defs = "ifndef TARGET\nTARGET=native\nendif";	
	private static String INSENSE_RUNTIME_UNIX_CHECK = "ifndef INSENSE_RUNTIME_UNIX\n     $(error INSENSE_RUNTIME_UNIX not defined! You must specify where INSENSE_RUNTIME_UNIX resides!)\nendif";
	//private static String MSP430_GCC_VER_CHECK = ""; //"ifndef MSP430_GCC_VER\n     $(error MSP430_GCC_VER not defined! You must specify MSP430_GCC_VER as e.g. 323 for gcc 3.2.3 or 453 for 4.5.3!)\nendif";
	...
	//private static String APP_INCLUDES = "APP_INCLUDES = -I $(INSENSE_RUNTIME_UNIX) -I $(INSENSE_RUNTIME_UNIX)/$(DAL_OS_DIR)";
	// JL altered above to following
	private static String APP_INCLUDES = "APP_INCLUDES = -I $(INSENSE_RUNTIME_UNIX)";
	...
	private static String EXTRA_OBJECTFILES = "# JL added, linking to main function in contiki-sky.a doesn't seem to work\n"+
											"# with msp430-gcc\n" + 
											"ifeq ($(TARGET), sky)\n" + 
	...
	private static String ALL = "all: iclean $(APP_OBJECTFILES) insense_runtime_unix.a $(PROJECT_LIBRARIES)\n\t$(LD) $(LDFLAGS) $(APP_OBJECTFILES) $(EXTRA_OBJECTFILES) insense_runtime_unix.a -o ";
	...
	MakefileUnix( String projectName ) {
...
private void printCompilerControlledRuntimeComponentFlags(PrintStream ps){
	...
	public void generateMakeFile() { 
		try {
			...
			ps.println( defs );
			ps.println();
			ps.println( INSENSE_RUNTIME_UNIX_CHECK );
			...
			ps.println();			
			ps.println( "PROJ=" + projectName.replaceAll(".*/","") + Code.NEWLINE ); //AB: just use basename
			...
			ps.println( EXTRA_OBJECTFILES );
			ps.println();
			ps.print( ALL ); ps.println( "$(PROJ).$(TARGET)" );
			ps.println();
			ps.println( INCLUDE );
