...
import uk.ac.stand.cs.insense.compiler.cgen.IComponentDereference;
...
public class Cgen implements ICodeGenerator {
	...
	public Cgen( ICompilerErrors ce, String project_name ) {
		...
		instance = this;
		makefile = new MakefileUnix( project_name );
	}
	...
	public ICode constructorBody(FunctionType ft, List<String> names, int scope_level ) {
		IConstructor newbody = new Constructor( ( (IProcedureContainer) findEnclosingDelcarationContainer()), ft, names, findEnclosingDelcarationContainer(), scope_level );
	...
	public void setRuntimeComponentFlags(String code){
        ...
        if(code.contains("buttonSensor" + glob_suffix)) MakefileUnix.setDAL_BUTTON(true);
        if( code.contains("lightHumidTempSensor" + glob_suffix) ) 
            		MakefileUnix.setDAL_SENSORS(true);
        if(code.contains("leds" + glob_suffix)) MakefileUnix.setDAL_LEDS(true);
        if(code.contains("standardOut" + glob_suffix)) MakefileUnix.setDAL_STDOUT(true);
        if(code.contains("radioIn" + glob_suffix) ||
        		code.contains("radioOut" + glob_suffix) ||
        		code.contains("radio" + glob_suffix) ||
        		code.contains("setRadioPower")) 
        	MakefileUnix.setDAL_RADIO(true);
        if(code.contains("radioIn") ||
        		code.contains("setRadioPower")){
        	MakefileUnix.setDAL_RECEIVE(true);
        }
        if(code.contains("publishOut") || code.contains("publishIn") 
        		|| code.contains("bind") || code.contains("unbind")
        		|| code.contains("getNumberHopsTo")
        		|| code.contains("findNodesPublishing")
        		|| code.contains("getNeighbours")
        		|| code.contains("getNeighboursOf")
        		|| code.contains("getPublicChannels")
        		|| code.contains("getPublicChannelsOf")
        		|| code.contains("enableInterNodeChannelRouting"))
        	MakefileUnix.setDAL_INTERNODECHANNEL(true);
        if(code.contains("periodicEnSchedule")){
        	MakefileUnix.setDAL_SCHEDULER(true);
        }
