...
public class Component extends ProcedureContainer implements ICode, IComponent {
	...
	private String initGlobalsName;
	private ICode behaviour;
	private boolean is_complete;

	private HashMap<String,ChannelType> channels;
	private ConcurrentSkipListSet<String> channel_names; 


	private List<IConstructor> constructors;
	private String behaviour_name;
	private String process_name;
	private String outputted_component_name;
	private List<String> externalIncludes;
...
//		addExternalIncludes( Code.HASH_INCLUDE_ + Code.DQUOTE + channelIncludeName( tr ) + Code.DQUOTE); // TODO NOT NEEDED INCEOS?
	...
	public void addInterface(String name, InterfaceType type) {
		for( TypeName iter : type ) {
			if( iter.type instanceof ChannelType ) {
				addChannel( iter.name + "_comp", (ChannelType) iter.type );
			} else {
	...
	*/
	 ...
	 * @param ps - the stream on which the decls are written
	 * @param ft - the typeof the (constructor) function
		...
		for( IDecl l : locations ) {
			...
			if(tr.isPointerType()){ 
				if(tr instanceof ChannelType && l.getSymbolTableEntry().getContext() > ISymbolTable.COMPONENT){
					...
					//sb.append(tab(2) + functionCall("Call_unbind_proc", THIS+ARROW+"process_pntr", NULL, "inch_comp" + ARROW + "node_addr", l.getSymbolTableEntry().contextualName(ISymbolTable.COMPONENT) + ARROW + "LCN") + SEMI + NEWLINE);
					...
					sb.append( TAB + functionCall( "// TODO need InceOS Channel_decRef" , l.getSymbolTableEntry().contextualName(ISymbolTable.COMPONENT) ) + SEMI + NEWLINE );
	...
	/**
	 * @param ps - the stream on which the initialisers are written
...
//	}
	 ...
	 * @param ps - the stream on which the decls are written
	 * @param ft - the function whose parmater list is being generated
	 */	
