...
public class Dereference extends Location implements IDereference {
	...
	private List<Deref> dereferences = new ArrayList<Deref>();	// Lists the multiple dereferences
	...
	public void channelDereference(String the_name) {
		if( ste.getType() instanceof ComponentType) {
			...
			// was current = new Deref( CHANNEL, "impl->get_" + the_name + "_comp" + LRB + ste.contextualName(fromContext) + RRB ); // top level component channel - so need to accomodate Mangling
		} else {
	...
	public void addArrayDeref( boolean last, Deref d ) {
		...
		if( last && lhs ) {	// this is the last dereference and we have an assignment so generate code to create a location.
			...
			lappend( TAB + LRB + functionCall( fn, subject, d.deref, AMPERSAND + "success" ) + RRB);
		} else {
...
//			lappend(STAR);
		...
		for( Deref d : dereferences ) {
			switch( d.kind ) {
				case CHANNEL: addChannelDeref( d.deref ); break;
				case FIELD:	addFieldDeref( d.deref ); break;
				case ARRAY: addArrayDeref( count == size, d ); break;
				case ARRAY_LENGTH: addArrayLengthDeref(d); break;
				default: throw new RuntimeException( "unexpected kind tag in dereference" );
			}
			count++;
		}
