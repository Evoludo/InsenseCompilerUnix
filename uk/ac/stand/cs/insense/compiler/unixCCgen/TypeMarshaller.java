...
import uk.ac.stand.cs.insense.compiler.unixCCgen.fileHandling.HeaderFile;
import uk.ac.stand.cs.insense.compiler.unixCCgen.fileHandling.ImplFile;
import uk.ac.stand.cs.insense.compiler.unixCCgen.fileHandling.OutputFile;
...
public class TypeMarshaller extends Code {
	 ...
	 * @param type - the type for which a serializer must be generated
	 */
	 ...
	 * @param type - the type for which a deserializer must be generated
	 */
	...
	private static String serializeValue( ITypeRep t, String from, boolean increment_pntr, boolean in_array, boolean in_struct ) {
		...
		if( t instanceof BooleanType ||
			...
			t instanceof UnsignedIntegerType ||
			t instanceof EnumType ) {
			check_space(sb, functionCall( "sizeof", "int" ) );
			sb.append( TAB + functionCall( MEMCPY, CHARSTARCAST + mem_block_name, memcpy_scalar_from_deref + from, functionCall( "sizeof", "int" ) ) + SEMI + NEWLINE );
			if( increment_pntr ) {
				sb.append( TAB +  mem_block_name + SPACE + EQUALS_ + LRB + CHARSTARCAST + mem_block_name  + RRB + "+" + functionCall( "sizeof", "int" ) + SEMI + TAB + NEWLINE );
			}
		} else
		if( t instanceof ByteType ) {
			check_space(sb, functionCall( "sizeof", "char" ) );
			sb.append( TAB + functionCall( MEMCPY, CHARSTARCAST + mem_block_name, memcpy_scalar_from_deref + from, functionCall( "sizeof", "char" ) ) + SEMI + NEWLINE );
			if( increment_pntr ) {
				sb.append( TAB +  mem_block_name + SPACE + EQUALS_ + LRB + CHARSTARCAST + mem_block_name  + RRB + "+" + functionCall( "sizeof", "char" ) + SEMI + TAB + NEWLINE );
			}
		}  else
		if( t instanceof RealType ) {
			check_space(sb, functionCall( "sizeof", "float" ) );
			sb.append( TAB + functionCall( MEMCPY, CHARSTARCAST + mem_block_name, memcpy_scalar_from_deref + from, functionCall( "sizeof", "float" ) ) + SEMI + NEWLINE );
			if( increment_pntr ) {
				sb.append( TAB +  mem_block_name + SPACE + EQUALS_ + LRB + CHARSTARCAST + mem_block_name  + RRB + "+" + functionCall( "sizeof", "float" ) + SEMI + TAB + NEWLINE );
			}
		} else
		// pointers
		if( t instanceof StringType ) {
			...
			if( increment_pntr ) {
				sb.append( TAB +  mem_block_name + SPACE + EQUALS_ + LRB + CHARSTARCAST + mem_block_name  + RRB + "+" + from + ARROW + "length + 1"  + SEMI + TAB + NEWLINE );
			}
		...
		if( t instanceof AnyType ) { // TODO JL if we want to continue allowing ANYs in STRUCTs then we will have to add size field to all deserialisers to permit the start of the next struct field to find in the serial data
	...
	private static String arraySerialize( StringBuffer sb, ArrayType t, String from ) {
		...
		sb.append(TAB + FOR + SPACE + LRB + current_loop_var() + EQUALS + ZERO + SEMI + SPACE + current_loop_var() + "<" + from + ARROW + "length" + SEMI + SPACE + current_loop_var() + PLUS_PLUS_ + RRB + LCB_ + NEWLINE);
		sb.append( serializeValue( tr, LRB + functionCall(ArrayOps.array_deref_function(tr), from, next_loop_var(), SPACE + AMPERSAND + "success") + RRB, true , true, false) );
		sb.append(TAB + RCB_ + NEWLINE);
		return sb.toString();
	}
	...
	private static String deserializeValue( ITypeRep t, String to, String from, boolean increment_pntr , boolean in_array) {
		...
		if( t instanceof BooleanType ||
			...
			t instanceof UnsignedIntegerType ||
			t instanceof EnumType ) {
			if(!in_array) sb.append( TAB + insenseTypeToCTypeName(t) + to + SEMI +NEWLINE );
		...
		if( t instanceof StringType ) {
			...
			if( increment_pntr ) {
				sb.append( TAB +  PARAMNAME + SPACE + EQUALS + LRB + CHARSTARCAST + PARAMNAME  + RRB + "+" + functionCall( "strlen", from ) + PLUS_ + "1" + SEMI  + NEWLINE  );
			}	
		} else 
		if( t instanceof StructType ) {
			if(!in_array) sb.append( TAB + insenseTypeToCTypeName(t) + to + SEMI +NEWLINE );
			...
			if(to.contains("array_loc")){
				sb.append( TAB + functionCall("DAL_assign", AMPERSAND + to, SPACE + functionCall(  StructDeclaration.constructor_name( (StructType) t ), structActualParams(count,(StructType) t) ) ) + SEMI + NEWLINE );
			} else {
				sb.append( TAB + to + SPACE + EQUALS_ + functionCall(  StructDeclaration.constructor_name( (StructType) t ), structActualParams(count,(StructType) t) ) + SEMI + NEWLINE );
			}
		...
		if( t instanceof ArrayType ) {
			if(!in_array){
				...
				sb.append( functionCall( generate_unique_constructor_name((ArrayType)t), arrayLengths((ArrayType) t), NULL) + SEMI + NEWLINE );
			}
	...
	private static String arrayLengths(ArrayType t){
		StringBuffer sb = new StringBuffer();
		int dims = ((ArrayType) t).getDimensionality();
		int i;
		for(i=0; i<dims-1; i++){
	...
	private static String arrayDeSerialize( StringBuffer sb, ArrayType t, String to, String from ) {
		ITypeRep tr = t.getArray_type();
		sb.append(TAB + "unsigned " + current_loop_var() + SEMI + NEWLINE);
		sb.append(TAB + FOR + SPACE + LRB + current_loop_var() + EQUALS + ZERO + SEMI + SPACE + current_loop_var() + "<" + "length"+length_var_counter++ + SEMI + SPACE + current_loop_var() + PLUS_PLUS_ + RRB + LCB_ + NEWLINE);
		sb.append( deserializeValue( tr, LRB + functionCall(ArrayOps.array_deref_function(tr), to, next_loop_var(), SPACE + AMPERSAND + "success") + RRB, from, true , true) );
		sb.append(TAB + RCB_ + NEWLINE);
		length_var_counter--;
		return sb.toString();
	}
	...
	private static String deSerializeArrayLengths(ArrayType t){
		...
		for(int i=0; i<t.getDimensionality(); i++){
			...
			sb.append(TAB + functionCall(MEMCPY, CHARSTARCAST + AMPERSAND + lengthVarName(i), SPACE + CHARSTARCAST + PARAMNAME, " sizeof(int)") + SEMI + NEWLINE);
	...
	private static void printDeSerializer( ITypeRep t,  PrintStream out ) {
		...
		if( t instanceof StringType ) {	// Strings
			//sb.append( TAB + insenseTypeToCTypeName(t) + "value = 0;" + NEWLINE );
			sb.append( deserializeValue( t, "value", PARAMNAME, false, false ) ); // type, to , from
			sb.append( TAB + "result = " + functionCall( constructAny( t ), "value", DQUOTE + t.toStringRep() + DQUOTE ) + SEMI + NEWLINE );
		} else 
	...
	private static void printInitializeSerializerFunctions( PrintStream out ) {
		...
		for( ITypeRep t : encounteredTypesRequiringSerialization ) {
			...
			if( encounteredTypeStringsRequiringDeSerialization.contains(t.toStringRep()) ) { 	// we have a pair to put in the table
				sb.append( TAB + functionCall( "mapPut",
											   serialiserMap,  						// the map
	...
	private boolean requireGeneration() {
		return ! (  encounteredTypesRequiringSerialization.isEmpty() &&
				    encounteredTypesRequiringDeSerialization.isEmpty() );
	}
