...
import uk.ac.stand.cs.insense.compiler.CompilerErrors;
...
public class ArrayConstructor extends ArrayOps implements ICode, IArrayConstructor {
	...
	private ICompilerErrors compilerErrors;
	...
	public void complete() {
		...
		for( String size : dimensions ) {
			...
			params.append( size );			params.append( COMMA );		
		}
	...
	protected static String constructorCode(ITypeRep array_type){
		...
		int index = 0;
		for(index=0; index<num_dimensions; index++) // add the params - one for each dimension
		...
		for( int dim = num_dimensions - 1 ; dim >= 0; dim-- ) {
			...
			if( body == null ) { // the starting case
				...
				body = tab(dim+1) + "IArrayPNTR" + SPACE + variable + SPACE + EQUALS + SPACE + 
						functionCall( array_constructor_name( base_type ), paramName( dim ), functionCall("sizeof", insenseTypeToCTypeName(final_base_type) ) , SPACE + initialiser, contains_pointers ) + SEMI + NEWLINE;
			} else {
				StringBuffer sb = new StringBuffer();
				sb.append( tab(dim+1) + " int " + loopName( count ) + SEMI + NEWLINE);
				sb.append( tab(dim+1) + "IArrayPNTR" + SPACE + variable + SPACE + EQUALS + SPACE + functionCall( array_constructor_name( base_type ), paramName( dim ), functionCall("sizeof", insenseTypeToCTypeName(base_type) ) , initialiser , contains_pointers  ) + SEMI + NEWLINE );
				sb.append( tab(dim+1) + FOR + LRB + loopName( count ) + EQUALS + ZERO + SEMI + loopName( count ) + LT_ + paramName( dim ) + SEMI + loopName( count ) + PLUS_PLUS_ );
				sb.append( tab(dim+1) + RRB + LCB_ + NEWLINE );
				sb.append( body );
				//sb.append( tab(dim+2) + functionCall( array_lhs_deref_function( type ), variable, loopName( count ),AMPERSAND + "success" ) + " = " + last_variable + SEMI + NEWLINE );
				// JL mod
				sb.append( tab(dim+2) + functionCall("DAL_assign",AMPERSAND+functionCall( array_lhs_deref_function( base_type ), variable, loopName( count ),AMPERSAND + "success" ) + COMMA + last_variable )+ SEMI + NEWLINE );
	...
	protected static String copyFunctionCode(ITypeRep array_type){
		...
		for( int i=0; i<num_dimensions; i++){
			...
			buff.append( tab(count+1) + FOR + LRB + loop_counter + EQUALS + ZERO + SEMI + SPACE + loop_counter + "<" + parameters.get(count) + SEMI + SPACE + loop_counter + "++" + RRB + LCB_ + NEWLINE);
			count++;
		}
		...
		for(int count2 = 0 ; count2 < count ; count2++)
			buff.append(COMMA + SPACE + loop_counter_prefix + count2 + COMMA + SPACE + AMPERSAND + "success" + RRB);
		if(final_base_type.isPointerType()){
	 ...
	 * @param constructor_name the name of the constructor that is generated
	 */
	 ...
	 * @param copy_function_name the name of the function that is generated
	 * @param constructor_name the name of the constructor for this array type
	 */
